    ...
    loading(cards: JobCardDataFormat[]) {
      for (let i = 0; i < cards.length; i++) {
        switch (cards[i].status) {
          case 'Todo':
            this.loadCardsInRespectiveColumns(cards[i], this.columns[0])
            break
          case 'In Progress':
            this.loadCardsInRespectiveColumns(cards[i], this.columns[1])
            break
          case 'Awaiting review':
            this.loadCardsInRespectiveColumns(cards[i], this.columns[2])
            break
          case 'Done':
            this.loadCardsInRespectiveColumns(cards[i], this.columns[3])
            break
          default:
      ...
      this.N_M_Sort(this.columns[1].cards, this.order_of_sorting_in_columns)
      this.N_M_Sort(this.columns[2].cards, this.order_of_sorting_in_columns)
      this.N_M_Sort(this.columns[3].cards, this.order_of_sorting_in_columns)
