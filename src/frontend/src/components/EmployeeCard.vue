      ...
      >
        <v-card>
          <v-card-text>
            <v-list two-line>
              <v-list-item-avatar>
                <v-img :src="employee.profile.displayImage" height="100" alt="Profile image"></v-img>
              </v-list-item-avatar>
              <v-list-item-content>
          ...
          <v-card-actions>
            ...
            <v-chip-group>
              <v-chip v-if="employee.skills.length > 0" color="primary" text-color="white">
                ...
                <span v-for="(skill, idx) in employee.skills.slice(0, 2)" :key="idx">{{ skill.skillName }}</span>
                <span v-if="employee.skills.length > 2"> + {{ employee.skills.length - 2 }} more</span>
              </v-chip>
              <v-chip v-if="employee.currentJobAssignments.length > 0" color="secondary" text-color="white">
  ...
  data() {
    return {
      employees: [
        {
          profile: {
            displayImage: "https://www.google.com/url?sa=i&url=https%3A%2F%2Fwww.freepik.com%2Ffree-photos-vectors%2Fblack-man-face&psig=AOvVaw3tTPSAhX8pBZPvzJxYQQe9&ust=1718972540927000&source=images&cd=vfe&opi=89978449&ved=0CBQQjRxqFwoTCPirqO-V6oYDFQAAAAAdAAAAABAE",
