<template><div><h1 id="quality-requirements" tabindex="-1"><a class="header-anchor" href="#quality-requirements"><span>Quality Requirements</span></a></h1>
<p>The quality requirements, in order of priority, that have been identified are as follows:</p>
<ul>
<li>Quality Requiremnets
<ul>
...
<p>The usability of our frontend is measured by how easily users can interact with it. The users should be able to easily navigate through the application, with</p>
<p>Users should be able to quickly track their schedule and open communication channels. Users in managerial positions should have a concise view of relevant information about their subordinates' activities.</p>
...
<p>The reliability of the system of the system is primarily measured by <strong>mean time between failures</strong> (MTBF) and <strong>mean time to recovery</strong> (MTTR). The number of critical failures per month will also be used as another metric to quantify the reliability of the system.</p>
<p>Asynchronous messaging between components in the system will allow the system to continue running smoothly under increased loads and handle intermittent failures more effectively.</p>
...
<p>Security will be quantified through measuring the confidentiality and integrity of the system. As such, we plan to implement a Gatekeeper that will serve as a broker between the client requests and the services, to sanitize and validate the requests.</p>
...
<p>The performance of the app will be measured using the following criteria:</p>
<ol>
<li><strong>Latency</strong> and <strong>Throughput</strong>: The time taken for a request to be processed and a response to be delivered. Low latency ensures real-time communication. The number of requests the system can handle per unit of time.</li>
<li><strong>Resource Utilization</strong>: Monitoring CPU, memory, and network usage to ensure efficient resource allocation. Mobile devices have limited resources, so optimizing utilization is critical.</li>
</ol>
